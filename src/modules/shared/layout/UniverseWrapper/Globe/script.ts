...
export const gitHubGlobe = () => {
  ...
  if (container) {
    renderer.setClearColor(0xff006e, 0)
    renderer.setSize(container.offsetHeight, container.offsetHeight)
  }
  scene.background = null
  document.body.appendChild(renderer.domElement)

  var light1 = new THREE.PointLight(0x48cae4, 0.75)
  light1.position.set(-150, 150, -50)

  var light2 = new THREE.PointLight(0xff006e, 0.75)
  ...
  function GrowTube(index: number, renderCount: number) {
    renderCount = Math.ceil(renderCount / 3) * 1
